<template>
    <div class="flex">
    <div v-for="artist in artistArray" :key="artist.artistId" class="flex-col w-1/4 text-center">
      <div class="flex-auto border-white border-2 border-t-0">
      <div v-on:click.prevent="goToArtistPage(artist.artistId)" class="block lg:text-xl md:text-xs sm:text-xs break-words text-center">
        {{ artist.artistName }}
...
@Options({
  ...
  methods: {
    async getRequest () {
      try {
        const res = await axios.post(DataBaseConnection, { query: '{ artists { artistId, artistName, photo, founded, hometown } }' }, options)
